...
enviro_clean <- enviro %>% 
  ...
  mutate(lowbirthweight = as.numeric(lowbirthweight),
         ...
         weight = totalpopulation/sum(totalpopulation)) #%>%
  #drop_na(`ces4.0score`)
...
enviro_clean_county <- enviro_clean %>%
  ...
  summarize(totalpop = sum(totalpopulation),
            ...
            avg_pollutionburden = weighted.mean(pollutionburden, weight),
            avg_asthma = weighted.mean(asthma, weight),
            avg_lowbirthweight = weighted.mean(lowbirthweight, weight),
            avg_cvd = weighted.mean(cardiovasculardisease, weight),
            avg_education = weighted.mean(education, weight),
            avg_lingisolation = weighted.mean(linguisticisolation, weight),
            avg_poverty = weighted.mean(poverty, weight),
            avg_unemployment = weighted.mean(unemployment, weight),
            avg_housingburden = weighted.mean(housingburden, weight)
            )
```
...
enviro_clean_asthma <- enviro_clean %>%
  select(county, totalpopulation, `ces4.0score`, `pm2.5`, dieselpm, pesticides, traffic, pollutionburden, asthma, cardiovasculardisease, poverty, housingburden, weight) %>% 
  group_by(county) %>% 
  summarize(totalpop = sum(totalpopulation), 
            #avg_ces_score = round(weighted.mean(`ces4.0score`, weight),2),
            avg_pm2.5 = round(weighted.mean(`pm2.5`, weight),2),
            avg_diesel = round(weighted.mean(dieselpm, weight),2),
            avg_pesticides = round(weighted.mean(pesticides, weight),2),
            avg_traffic = round(weighted.mean(traffic, weight),2),
            avg_pollutionburden = round(weighted.mean(pollutionburden, weight),2),
            avg_asthma = round(weighted.mean(asthma, weight),2),
            avg_cvd = round(weighted.mean(cardiovasculardisease, weight),2),
            avg_poverty = round(weighted.mean(poverty, weight),2)
            )
```
