---
title: "Making Visualizations with Cal Enviroscreen 4.0"
...
I found it valuable as an MPH student to work with manageable, real-world data sets as I got more comfortable using R. By manageable I mean the data set is large enough to be interesting but not so big to be overwhelming, has good documentation, and data that requires some but not a lot of data cleansing. It was also very important to me that the data could be used to develop meaningful insights of an existing population.
...
Here are a few examples of data visualizations I created looking at just couple of variables.
...
1.  **Scatter plot - Average CES 4.0 score and percentage of population living in poverty:** I grouped by county and then calculated population, mean CES 4.0 score, and mean poverty (%), weighted for population count. To calculate the CES 4.0 score, CalEnviroScreen 4.0 uses 20 different indicators to rank each census tract by its cumulative pollution burden and associated social vulnerabilities (e.g., poverty, education level) that put the population at risk. A higher score indicates higher risk to health concerns due to these pollutants and social indicators. Poverty is defined as the percent of population living below two times the federal poverty level (OEHHA, 2021).
...
suppressMessages(rural_plot_2_interact)
```

2.  **Box Plots - distribution of CES 4.0 scores and poverty by rural county status.**

Next I looked at how these counties are distributed when you take into consideration their rural status. 41 of California's 58 counties are considered rural.

The median values for CES 4.0 score did not differ greatly by rural status (23.71 non rural compared to 20.84 rural). The rural counties (41) had more observations and more outliers.

However, there's a significant difference in distribution in terms of poverty. The median percentage of poverty in non-rural counties is 28.82% and much higher in rural counties at 35.63%.
...
Finally, I plotted the association between average CES 4.0 score and average percentage of population in poverty, considering rural status. This visualization illustrates well the rural-urban poverty divide. There are two distinct distributions here in terms of rural status.
...
-   The association between CES 4.0 score and poverty is more pronounced in non urban counties with larger populations (e.g., Los Angeles county) at higher risk than smaller, non urban population.
